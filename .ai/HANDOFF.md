...
- Benchmark assumptions + dictionary + zstd experiment models are centralized in `scripts/benchmark/contract.py` and embedded into run output (`assumptions` + `dataset_manifest` + `dictionary_artifacts` + per-run `dictionary` + per-run `zstd_level`/`zstd_strategy`).
