...
Phase 2.2 normalized comparative mapping/reporting over raw execution evidence under `PHASE2_RESEARCH/generated/execution/`.

## First actions for a fresh instance

1. Read startup order from `../AI_BOOTSTRAP.md`.
2. Confirm `STATUS.md` and `PHASE_PLAN.md` both show CRUSHR-P2-TRIAL-READY-01 complete and Phase 2.2 comparative mapping/reporting as next.
...
5. Consume `crushr-lab` Phase 2 manifest/scenario enumeration plus deterministic audit/foundation/execution commands (`run-phase2-pretrial-audit`, `build-phase2-foundation`, `run-phase2-execution`) as canonical Phase 2 inputs; do not mutate locked matrix semantics.
...
- CRUSHR-P2-TRIAL-READY-01 finished: added `run-phase2-pretrial-audit` with deterministic matrix/tool/file/output-root readiness checks, failure categories, machine-readable `phase2_pretrial_readiness_v1` report output, focused audit tests, and canonical operator-flow docs update in `PHASE2_RESEARCH/README.md`.
...
Map raw run records into normalized comparative results and publication summaries (Phase 2.2).
