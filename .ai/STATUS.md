...
- **Permitted next action:** execute baseline + dictionary + zstd level/strategy + ordering/locality experiment benchmark matrices in an environment with full comparator dependencies present and publish updated evidence artifacts from normalized schema.
- Future packets may assume:
  ...
  - benchmark output includes embedded dataset identity + assumptions metadata + dictionary provenance metadata + zstd level/strategy metadata + ordering strategy metadata,
