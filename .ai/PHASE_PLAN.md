...
- [x] P17S02f4 — align README command-surface/product-boundary text with canonical CLI (`info`, `extract`, `verify`, `pack`, `about`, `completion`, `man`) including `info --propagation` coverage and concise command descriptions.
...
- Version governance remains locked: `VERSION` is canonical, user-facing/runtime version output must match accepted repo truth, and bumps follow the established project policy only.
...
- [x] P19S02f2 — correct corruption demo semantics by replacing off0-heavy defaults with bounded demo presets, separating kill-shot modes from default flow, improving mode descriptions, and validating outcome spread with canonical CLI checks.
