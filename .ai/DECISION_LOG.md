...
## 2026-04-03 — P19S01f2 WASM pack output byte-normalization lock

- Decision:
  - Treat wasm-returned `archive_bytes` as an adapter boundary requiring explicit JS-side byte normalization before blob download emission.
  - Add strict conversion helper supporting typed-array, ArrayBuffer, and numeric-array forms; reject unsupported forms explicitly.
  - Add a Rust regression test that validates demo-generated bytes through canonical CLI `info` and `info --list` commands.
- Alternatives considered:
  1. Keep implicit blob conversion and only adjust Rust emission.
  2. Introduce a JS-side archive assembly fallback.
- Rationale:
  - Invalid archives were caused at the browser/WASM adapter boundary, not by core Rust `.crs` assembly semantics.
  - Explicit byte normalization removes representation ambiguity and preserves Rust as the archive source of truth.
- Blast radius:
