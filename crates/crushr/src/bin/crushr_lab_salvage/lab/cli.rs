...
pub(super) fn parse_cli_options() -> Result<CliOptions> {
    ...
    if let Some(first) = args.next() {
        ...
        if first == "run-file-identity-comparison"
            ...
            || first == "run-format12-inline-path-comparison"
            ...
            || first == "run-format15-comparison"
            || first == "run-format15-stress-comparison"
        {
            let mut output_dir = None;
            let mut verbose = false;
            while let Some(arg) = args.next() {
            ...
            return Ok(CliOptions {
                ...
                } else if first == "run-format12-inline-path-comparison" {
                    Mode::RunFormat12InlinePathComparison {
                        comparison_dir: output_dir.context(USAGE)?,
                    }
                } else if first == "run-format12-stress-comparison" {
                    Mode::RunFormat12StressComparison {
                        comparison_dir: output_dir.context(USAGE)?,
                    }
                } else if first == "run-format13-comparison" {
                ...
                } else if first == "run-format15-comparison" {
                    Mode::RunFormat15Comparison {
                        comparison_dir: output_dir.context(USAGE)?,
                    }
                } else if first == "run-format15-stress-comparison" {
